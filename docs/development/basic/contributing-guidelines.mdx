...
- **Human** — what changed, screenshots, how you tested it, the acceptance round, and the related issue (`Fixes #123`, `Fixes LOBE-xxx`)
- **AI** — delete this section when a person wrote the change. When an agent wrote it, and the author is an outside collaborator or another external contributor, the commented instructions in the template tell that agent to record the harness, the model, the thinking level, and every user prompt from the session, in order. The author reviews that text before it is published, and sensitive content is redacted. An active member of the lobehub organization deletes this section.
