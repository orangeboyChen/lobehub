...
- **Human** — 改了什么、截图、如何测试、验收记录，以及关联 issue（`Fixes #123`、`Fixes LOBE-xxx`）
- **AI** — 人写的改动删掉这一节。Agent 写的改动，如果作者是外部 collaborator 或其他外部贡献者，模板里的注释指令会要求这个 agent 填写 harness、模型、thinking level，以及这次会话里的全部 user prompt（按顺序保留）。发布前由作者审阅这段文字，敏感内容打码。lobehub organization 的正式成员删掉这一节。
