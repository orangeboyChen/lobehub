 ...
 */
export const normalizeChatMessageError = (error: unknown): ChatMessageError => {
  if (
    isRecord(error) &&
    (typeof error.errorType === 'string' || typeof error.errorType === 'number')
  ) {
    const payload: ChatCompletionErrorPayloadLike = {
      ...error,
      errorType: error.errorType as ChatMessageError['type'],
